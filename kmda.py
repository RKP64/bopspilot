...
async def get_query_plan_from_llm(
    ...
    planning_prompt = f"""You are a query planning assistant for analyzing BIAL Operational data.
...
# --- CHAT HANDLER (Original version - kept) ---
async def handle_chat_request(request: ChatRequest, current_user: User):
    """Handles both chat endpoints using web search or standard RAG."""
    start_time = time.time()
    # Set context for tracing using attach within the request scope
    ctx = Context()  # Create a new context
    ctx = set_value(USER_ID_KEY, current_user.username, context=ctx)
    ctx = set_value(USER_ROLE_KEY, current_user.role or "user", context=ctx)
    ctx = set_value(USER_QUERY_KEY, request.question, context=ctx)
    token_otel = attach(ctx)  # Attach the context

    try:
        # Content Safety Check
        user_input_safety = await analyze_content_safety(request.question)
        if not user_input_safety.is_safe:
            raise HTTPException(
                400,
                f"Input blocked for safety: {', '.join(user_input_safety.categories)}",
        ...
        if not clients.synthesis_openai_client or not clients.planning_openai_client:
            raise HTTPException(503, "AI services not configured.")

        # Web Search Check (using original keywords)
        is_web_search = any(
            w in request.question.lower()
            for w in [
                "web search",
                "latest",
                "current",
        ...
        response_data = None
        source = "web" if is_web_search else "internal"

        if is_web_search:
            logging.info(
                f"Gemini Deep Research triggered for chat: '{request.question[:100]}...'"
            )

            if check_creds(config.gemini_deepsearch_api_key):
                final_answer = "Gemini Deep Research API key is not configured."
                ...
                web_context = await query_gemini_deep_research(
                    question=request.question,
                    api_key=config.gemini_deepsearch_api_key,
                )

                if not web_context or web_context.startswith("Error"):
                    final_answer = (
                        f"I tried running deep web research but it failed.\n\n{web_context}"
                    )
                    response_data = ConversationalChatResponse(
                        answer=final_answer,
                        plan=[request.question],
                    ...
                    web_synthesis_prompt = f"""
        Based ONLY on the following web research results, answer the user's question.

        Rules:
        - Do NOT add outside knowledge
        - Do NOT answer politics or religion unless directly asked
        ...
        """
                    ...
                    final_answer, _ = await generate_synthesis_answer(
                        ...
                        client_for_synthesis=clients.synthesis_openai_client,
                        synthesis_deployment_id=config.deployment_id,
                        system_prompt_override=web_synthesis_prompt,
                    )
            ...
            # Standard RAG Pipeline (internal index only)
            logging.info(
                f"Internal RAG triggered for chat: '{request.question[:100]}...'"
            )
        ...
        #         "serpapi",
        #     ]
        # )  # Added original keywords

        # response_data = None
        # source = "web" if is_web_search else "internal"  # Track source

        # if is_web_search:
        #     logging.info(
        #         f"Web search triggered for chat: '{request.question[:100]}...'"
        #     )
        #     # Determine which web search tool to use (SerpApi priority)
        #     web_context = (
        #         await query_serpapi(request.question)
        #         if not check_creds(config.serpapi_api_key)
        #         else ""
        #     )
        #     if not web_context or web_context.startswith("Error"):
        #         logging.info("SerpApi failed or not configured, trying Bing...")
        #         web_context = await query_bing_web_search(request.question)

        #     if not web_context or web_context.startswith("Error"):
        #         final_answer = f"I tried searching the web, but couldn't find relevant information. Error: {web_context}"
        #         response_data = ConversationalChatResponse(
        #             answer=final_answer,
        #             plan=[request.question],
        #             sources=[],
        ...
        #         web_synthesis_prompt = f"""Based ONLY on the following web search results, answer the user's question concisely. Do not add outside knowledge. striclty dont give any answer related to politics , religion unrelated to the question. Always answer in 1000 words. Give detailed information for each question.
        #          USER QUESTION: {request.question}
        #          SEARCH RESULTS:\n---\n{web_context}\n---\nCONCISE ANSWER:"""  # <<< Correct indentation and variable

        #         # Use generate_synthesis_answer for consistency, but with no doc/hist context
        #         final_answer, _ = await generate_synthesis_answer(
        #             user_question=request.question,
        #             document_context="",
        #             historical_context="",
        #             sources=[],  # No internal context/sources
        #             max_tokens_param=16000,  # Limit web answer length
        #             client_for_synthesis=clients.synthesis_openai_client,
        #             synthesis_deployment_id=config.deployment_id,
        #             system_prompt_override=web_synthesis_prompt,  # Pass the prompt
        #         )
        #         response_data = ConversationalChatResponse(
        #             answer=final_answer,
        #             plan=[request.question],
        ...
        #         # --- End Corrected Block ---
        # else:
        #     # Standard RAG Pipeline (using index only)
        #     logging.info(
        #         f"Internal RAG triggered for chat: '{request.question[:100]}...'"
        #     )
        #     # This calls the original RAG pipeline which includes planning and index search
        #     rag_result = await execute_advanced_rag_pipeline(
        #         request.question, request.history
        #     )
        #     response_data = ConversationalChatResponse(
        ...
        # Log interaction
        # Ensure response_data is not None before accessing attributes
        if response_data:
            await log_interaction(
                current_user.username,
                request.question,
            ...
            # This case should ideally not happen if logic covers all paths
            logging.error(
                "handle_chat_request reached end without setting response_data"
            )
        ...
        # Log interaction attempt with error message
        await log_interaction(
            current_user.username,
            request.question,
            f"Error: {str(e)}",
        ...
        if token_otel:  # Detach context only if it was attached
            detach(token_otel)
